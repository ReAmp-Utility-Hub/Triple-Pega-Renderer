...
function App() {
  ...
  const [caseDetails, setCaseDetails] = useState({ urgency: '', status: '', created: '', assignedTo: '', type: '' });
  ...
  const getAssignmentDetails = async (id) => {
    try {
      const response = await fetch(`${API_BASE}/assignments/${id}?viewType=form`, {
      ...
      setCaseDetails({
        ...
        created: caseInfo.createTime ? new Date(caseInfo.createTime).toLocaleDateString() : 'N/A',
      ...
      if (viewConfig && viewConfig.children?.[0]?.children) {
        ...
        const numericTypes = ['integer', 'decimal', 'currency', 'percentage', 'double'];
        ...
        fieldsArray.forEach(fieldObj => {
          if (fieldObj.config && fieldObj.config.value) {
            ...
            extractedFields.push({
              ...
              type: rawType,
              category: category,
              inputType: numericTypes.includes(typeLower) ? 'number' : (typeLower === 'checkbox' ? 'checkbox' : 'text'),
              label: fieldMetadata.label || fieldName,
              required: fieldObj.config.required === true || fieldMetadata.required === true,
  ...
  const submitAction = async (e) => {
    ...
    setValidationErrors([]);
    try {
      const response = await fetch(`${API_BASE}/assignments/${assignmentId}/actions/${actionId}?viewType=none`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      if (resData.confirmationNote) {
        ...
        setStep('FORM');
      } else if (resData.errorClassification === "Validation fail" || resData.errorClassification === "Invalid inputs") {
        setValidationErrors(resData.errorDetails);
        setStep('ASSIGNMENT_READY');
      } else if (!response.ok) {
        setValidationErrors([{ localizedValue: `Submission failed with status ${response.status}`, source: 'client' }]);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleRefresh = async (fieldName) => {
    try {
      const response = await fetch(`${API_BASE}/assignments/${assignmentId}/actions/${actionId}/refresh?refreshFor=.${fieldName}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      if (response.status === 204) {
        setValidationErrors(prev => prev.filter(e => e.erroneousInputOutputIdentifier !== `.${fieldName}`));
        return;
      }
      ...
      const errors = rawErrors
        ...
        .map(m => ({
          erroneousInputOutputIdentifier: m.path || m.erroneousInputOutputIdentifier || `.${fieldName}`,
          localizedValue: m.localizedValue || m.message,
          source: 'refresh'
        }));

      setValidationErrors(prev => {
        const withoutField = prev.filter(e => e.erroneousInputOutputIdentifier !== `.${fieldName}`);
    ...
    <div className="dashboard-wrapper">
      {step === 'INIT' && (
        <div className="loading-container fade-in">
          ...
          <button className="btn btn-primary" onClick={createCase} disabled={loading || !token}>
            {loading ? <div className="loading-spinner"></div> : "INITIALIZE ASSESSMENT"}
          </button>
        </div>
      )}
          ...
          <nav className="top-nav">
            <div className="nav-left">
              ...
              <div className="nav-meta">
                <span>Urgency <span className="badge">{caseDetails.urgency}</span></span>
                <span>Work Status <span className="badge">{caseDetails.status}</span></span>
                <span>Created By <strong>{caseDetails.assignedTo}</strong></span>
              </div>
            </div>
          </nav>
          <div className="app-body">
            <main className="main-content">
              <div className="form-container fade-in">
                {validationErrors.filter(e => e.source === 'refresh').length > 0 && (
                  <div className="global-error-box">
                    {validationErrors.filter(e => e.source === 'refresh').map((e, idx, arr) => (
                      <div key={idx} style={{ marginBottom: idx === arr.length - 1 ? 0 : '4px' }}>
                        {e.localizedValue || e.message}
                      </div>
                    ))}
                  </div>
                )}
                <h1>{layoutInfo.title} <span className="case-id-tag">{caseDetails.businessID}</span></h1>
                <p className="subtitle">{layoutInfo.instructions}</p>
                <form onSubmit={submitAction} noValidate>
                  <div className="dynamic-form-grid">
                    {viewFields.map(field => {
                      const isError = validationErrors.find(e => e.erroneousInputOutputIdentifier === `.${field.name}`);

                      // Handle custom QR Code component
                      if (field.category === 'qrcode') {
                        const qrValueProperty = field.config?.inputProperty?.replace('@P .', '') || field.name;
                        const qrValue = formData[qrValueProperty] || formData[field.name] || caseDetails.businessID || '';

                        return (
                          <div className="form-group" key={field.name} style={{ gridColumn: 'span 2', alignItems: 'center' }}>
                            ...
                            <div className="qr-code-wrapper">
                              ...
                              />
                              <span className="qr-value-text">{qrValue || 'Waiting for data...'}</span>
                            </div>
                          </div>
                        );
                      }
                      ...
                      return (
                        <div className={`form-group ${field.category === 'checkbox' ? 'checkbox-group' : ''}`} key={field.name}>
                          ...
                          <div className="input-wrapper">
                            {field.iconLeft && <span className="input-icon-left">{field.iconLeft}</span>}
                            ...
                            {field.category === 'select' ? (
                              ...
                              >
                                ...
                                {field.options.map(opt => (
                                  <option key={opt.key || opt.value} value={opt.key || opt.value}>{opt.value}</option>
                                ))}
                              </select>
                              ...
                              <input
                                ...
                                placeholder={field.label}
                                onChange={handleChange}
                                onFocus={() => setValidationErrors(prev => prev.filter(e => e.erroneousInputOutputIdentifier !== `.${field.name}`))}
                                onBlur={field.name.includes('RetirementAge') ? () => handleRefresh(field.name) : undefined}
                                autoComplete="off"
                                className={`${isError ? 'input-error' : ''} ${field.iconLeft ? 'has-icon-left' : ''} ${field.iconRight ? 'has-icon-right' : ''}`}
                              />
                            )}

                            {field.iconRight && <span className="input-icon-right">{field.iconRight}</span>}
                          </div>
                          {(isError && isError.source !== 'refresh') && <div className="error-message">{isError.localizedValue || isError.message}</div>}
                        </div>
                      );
                    })}
                  </div>
                  <div className="btn-group">
                    {buttons.secondary?.map((btn, i) => (
                      <button key={i} type="button" className="btn btn-secondary">{btn.name}</button>
                    ))}
                    {buttons.main?.map((btn, i) => (
                      <button key={i} type="submit" className="btn btn-primary" disabled={loading}>
                        {loading ? <div className="loading-spinner" style={{ width: '16px', height: '16px', borderWidth: '2px' }}></div> : btn.name}
                      </button>
                    ))}
                  </div>
                </form>
              ...
              <div className="details-container fade-in">
                ...
                <div className="details-grid">
                  <div className="sidebar-group">
                    ...
                    <span className="sidebar-value">{caseDetails.businessID}</span>
                  </div>
                  <div className="sidebar-group">
      ...
      {step === 'SUCCESS' && (
        <div className="loading-container fade-in">
          ...
          <button className="btn btn-secondary" onClick={() => setStep('INIT')}>RESET</button>
        </div>
      )}
    </div>
  );
